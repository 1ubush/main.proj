...
;(function($) {
  ...
  var cartData,
    ...
    orderform = '<p class="jqcart-cart-title">Контактна інформація:</p><form class="jqcart-orderform"><p><label>ФИО:</label><input type="text" name="user_name"></p><p><label>Телефон:</label><input id="phone" type="text" name="user_phone"></p><p><label>Email:</label><input id="email" type="text" name="user_mail"></p><p><label>Адреса:</label><input type="text" name="user_address"></p><p><label>Коментар:</label><textarea name="user_comment"></textarea></p><div style="text-align:left; padding-bottom:10px; font-size:16px; color: #BA0000;">Це важливо! замовлення приймаемо від 5 літрів в межах Киева</div><p><p><input type="submit" value="Замовити"><input type="reset" value="Повернутися до покупок"></p></form>';
  var opts = {
		buttons: '.add_item',
		cartLabel: 'body',
		visibleLabel: false,
  ...
  var actions = {
    ...
    openCart: function() {
      var subtotal = 0,
			cartHtml = '';
      cartData = actions.getStorage();
      orderPreview = '<p class="jqcart-cart-title">Кошик <span class="jqcart-print-order"></span></p><div class="jqcart-table-wrapper"><div class="jqcart-manage-order"><div class="jqcart-thead"><div>ID</div><div></div><div>Найменування</div><div>Ціна</div><div>Кіл-сть</div><div>Сума</div><div></div></div>';
      var key, sum = 0;
      for (key in cartData) {
        if (cartData.hasOwnProperty(key)) {
      ...
      orderPreview += '<div class="jqcart-subtotal">Разом: <strong>' + subtotal + '</strong> ' + opts.currency + '</div>';
			
			cartHtml = subtotal ? (orderPreview + orderform) : '<h2 style="font-size:22px" class="jqcart-empty-cart">Кошик пустый!</h2>';

      $(modal).appendTo('body').find('.jqcart-checkout').html(cartHtml).find('#phone').inputmask('mask', {
    ...
    sendOrder: function(e) {
      ...
      if ($.trim($('[name=user_name]', $that).val()) === '' || $.trim($('[name=user_phone]', $that).val()) === '') {
        ...
        // $('<p class="jqcart-error">Замовлення приймаємо від 5 літрів!</p>').insertBefore($that).delay(3000).fadeOut();
        return false;
      }
      if(orderSum < opts.minSum) {
      	// $('<p class="jqcart-error">Минимальная сумма заказа - ' + opts.minSum + opts.currency + '</p>')
      	$('<p class="jqcart-error">Приймаємо замовлення від 5 літрів</p>')
      	.insertBefore($that)
      	.delay(4000).fadeOut();
      	return false;
      }
      ...
      $.ajax({
        ...
        success: function(resp) {
          $('.jqcart-checkout').html('<p>' + resp.message + '</p>');
					if(!resp.errors) {
						setTimeout(methods.clearCart, 30000);
					}
        }
      });
    },
